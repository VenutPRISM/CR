...
#include "value.h"      // Znacheniya steka
#include "nt.h"         // Imya tablitsy
#include "icode.h"      // Promezhutochnyy kod
#include "lextypes.h"   // Tipy leksem
...
public:
    ...
    void push(const Value& v) throw (StackException) {
        if (depth >= max_size)
            extend();
    ...
    Value pop() throw (StackException) {
        if (depth <= 0)
    ...
    void pop(Value& v) throw (StackException) {
        if (depth <= 0)
            throw StackException("Stack empty");
    ...
    Value& top() throw (StackException) {
        if (depth <= 0)
            throw StackException("Stack empty");
        return stack_elements[depth - 1];
    }

    const Value& top() const throw (StackException) {
        if (depth <= 0)
            throw StackException("Stack empty");
        return stack_elements[depth - 1];
    }

    void extend() throw (StackException) {
